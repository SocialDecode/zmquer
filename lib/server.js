	...
	********************************/
	...
	var col = {},
		...
		r = {
			...
			**/
				var syncDB = function(){
					if (!r.ready || s_wk._zmq.pending > config.maxQueSize*0.5 || !enoughFreemem(config.minMem)) return;
			...
			**/
			...
			"listenJobs" : function(options){
				...
				setInterval(function(){
					//console.log("jobs:",jobs,options.max,s_wk_client._zmq.state);
					if ( s_wk_client._zmq.state !== 0 &&  jobs < options.max  && enoughFreemem(options.minMem) ) {
				...
				var on_message = function(msg){
					//one more job
					jobs++;
					if (s_wk_client._zmq.state == 0 &&  jobs >= options.max) s_wk_client.close();
					var c = JSON.parse(msg),
						args = "";
	...
	var enoughFreemem = function(limit){
		return 100-(os.freemem()/os.totalmem()*100) > limit;
	};
