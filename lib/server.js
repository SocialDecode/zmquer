						...
						case "couch":
							couchque.view('common','by_status',{
								...
								if (!geterr){
									...
									var docs = [];
									for (var i=0;i < getbody.rows.length;i++){
										if (!getbody.rows[i].doc) continue;
									...
									if (docs.length>0){
										//couchdb = nano(config.couch.server);
										var couchbulk = couchdb.db.use(config.couch.database);
										couchbulk.bulk({docs:docs}, function(errbulk,docsbulk){
											if (errbulk){
												console.log("Error while updating que jobs",errbulk);
											}
											console.log("_");
