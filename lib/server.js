...
var main = function(){
	var col = {},
		...
		r = {
			...
			"DBSyncInterval" : null,
			"initServer" : function (){
				var datalog = {gauge : function(){},legauge:function(){}}; //empty placeholder
				/**
				*
				...
				**/
				...
				if (config.datadog){
					...
					datalog = new statsD();
					datalog.socket.on("error", function(exception){
						console.log ("[datadog] error event in socked.send():"+exception);
					});
					datalog.legauges = {};
					datalog.legauge = function(type,host){
						var key = type+host;
						now = (~~((new Date()).getTime()/1000));
						slot = (Math.floor(now / 20) * 20)+"";
						if (!datalog.legauges[key]) datalog.legauges[key] = {};
						...
						jobsDone = 0;
						for (var ix in datalog.legauges[key]){
				...
				s_wc_client.on('message', function(msg){
					...
					if (c && c.node && c.node.name){
						...
						r.syncJobs(c.node.name,c.jobIds);
					}
					if (c.id){
					...
					if (c.node && c.node.name) datalog.gauge('task_processing', Object.keys(hostJobs[c.node.name]).length, ["worker:"+c.node.name]);
				...
				var getKeysCargo = async.cargo(function (tasks, callback){
					if (!r.ready  || getDocsCargo.length()>config.readBatchsize || couchDelCargo.length() > config.delBatchsize || couchDelCargoWorking) {
						...
						for(var i=0; i<tasks.length; i++){
							...
							couchque.view("queue","by_packepoch",task,function(geterr,getbody){
								if (geterr){
									...
									if (getbody.rows && getbody.rows.length>0){
										...
										if (getbody.rows.length<config.readKeySize){
											setImmediate(function () {
												getKeysCargo.push({limit: config.readKeySize,descending:"false"});
											});
										}else{
											setImmediate(
												(function(gb) {
										...
										setImmediate(function(){getKeysCargo.push({limit:config.readKeySize,descending:"false"});});
									}
								}
								callback();
							});
			...
			"syncJobs" : function(host,jobs){
				if (host && Array.isArray(jobs)){
					var now = ~~((new Date()).getTime()/1000);
					//add all the jobs the client sent it already has
					if (!hostJobs[host]) hostJobs[host] = {};
					jobs.forEach(function(el){
						if (!hostJobs[host][el]) hostJobs[host][el] = now;
						if (!couch_processing[el]) couch_processing[el] = 1;
					});
					//Job Pruning
					for (var nodeJob in hostJobs[host]){
						if (jobs.indexOf(nodeJob) <0 ) delete hostJobs[host][nodeJob];
					}
					for (var cpJob in couch_processing){
						var found = false;
						for (var ilhost in hostJobs){
							for (var iljob in hostJobs[ilhost]){
								if (cpJob == iljob) found = true;
							}
						}
						if (!found) delete couch_processing[cpJob];
					}
					// s_wk._outgoing = s_wk._outgoing.filter(function(item){
					// 	var found = false;
					// 	for (var cpJob in couch_processing){
					// 		if (item.toString('utf8').indexOf(cpJob) > -1) found = true;
					// 	}
					// 	return !found;
					// });
				}
			...
			**/
			...
			"listenJobs" : function(options){
				...
				s_wc.send(JSON.stringify({startup : os.hostname(),jobsIds:jobs}));
