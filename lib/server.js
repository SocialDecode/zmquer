					...
					if (c.id){
						...
						var doUpdate = function(){
							switch (dbtype){
								...
								break;
								case "couch":
									if (couch_processing[id]) {
										couch_processing[id].status = c.status;
									}else{
						...
						if (config.dumpSuccess && c.status == 3 && !c.error){
							switch (dbtype){
								...
								case "couch":
									if (!couch_processing[id]){
										couchque.update({deleted:true}, id,function(errup,res){
										if (errup) console.log("Error while updating job",id,"with status",c.status);
											console.log("x",id);
										});
									}else{
										couch_processing[id].status = 3;
										console.log("-",id);
										couchque.insert({"_deleted" : true,"_rev" : couch_processing[id].rev},id, function(errup,res){
