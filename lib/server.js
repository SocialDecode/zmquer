						...
						couchque.list(task,function(geterr,getbody){
							if (geterr){
								...
								if (getbody.rows && getbody.rows.length>0){
									...
									//run itself again
									if (getbody.rows.length==task.limit-1){
										getKeysCargo.push({limit:task.limit,startkey:getbody.rows[x].id});
									}else{
										getKeysCargo.push({limit:100});
									}
								}else{
									getKeysCargo.push({limit:100});
									callback();
			...
			var getDocsCargo = async.cargo(function(couch_toprocess, callback){
				if (!r.ready  || os.freemem() < (config.minMem*1048576)) {
					...
					couchque.list({
						...
						console.log("_");
						if (bulkerr) {console.log("error while getting DB data",bulkerr);getDocsCargo(couch_toprocess);callback();}
