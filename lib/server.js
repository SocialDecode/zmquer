						...
						case "couch":
							couchque.list({},function(geterr,getbody){
								...
								if (!geterr){
									...
									for (var x=0;x<getbody.rows.length && os.freemem() > (config.minMem*1048576);x++){
										var id = getbody.rows[x].id;
										if (!couch_processing[id]){
											couchque.get(
												id,
												function(errdoc,body){
													if (errdoc || !body) return;
													couch_processing[id] = 1;
													body["idtype"] = "string";
													s_wk.send(jsonpack.pack(body));
