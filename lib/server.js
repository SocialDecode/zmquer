...
var main = function(){
	var col = {},
		...
		datalog = {gauge : function(){},increment:function(){}}, //empty placeholder
					...
					if (c.id){
						...
						if (c.node.name && hostJobs[c.node.name]){
							if (c.status != 3){
								...
								datalog.increment('task_completed', ["worker:"+c.node.name]);
							}
						...
						var doUpdate = function(){
							...
							if (c.status==3 && c.error){
								...
								datalog.increment('task_errors', ["worker:"+c.node.name]);
							}
						};
