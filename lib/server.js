		...
		httpCB = {
			syncData : function(cb){
				cb({
					...
					servers : hostJobs,
					errors : Object.keys(couch_errors).length
				});
			},
			resetJobs : function(){
				couch_errors = {};
		...
		r = {
			...
			"initServer" : function (){
				...
				**/
				...
				var couchDelCargo =  async.cargo(function (tasks, callback){
					if (getDocsCargo.length()>0 && tasks.length < config.delBatchsize){
			...
			**/
			...
			var getKeysCargo = async.cargo(function (tasks, callback){
				if (!r.ready  || os.freemem() < (config.minMem*1048576) || getDocsCargo.length()>config.readBatchsize || couchDelCargo.length() > config.delBatchsize || couchDelCargoWorking) {
								...
								if (getbody.rows && getbody.rows.length>0){
									...
									for (var x=0;x<getbody.rows.length;x++){
										if (!couch_processing[getbody.rows[x].id] && !couch_cargo[getbody.rows[x].id]){
											couch_toprocess.push([getbody.rows[x].id,getbody.rows[x].value]);
											couch_cargo[getbody.rows[x].id] = true;
										}
									}
									if (couch_toprocess.length>0){ //sending workers
										console.timeEnd("getting "+task.limit+" keys");
									...
									if (getbody.rows.length<task.limit){
										setImmediate(function () {
											getKeysCargo.push({limit: config.readKeySize});
										});
									}else{
										setImmediate(
											(function(gb) {
												return function () {
													console.log(gb.rows[gb.rows.length - 1].id);
													getKeysCargo.push({
														limit: config.readKeySize,
														startkey :gb.rows[gb.rows.length - 1].key,
														startkey_docid: gb.rows[gb.rows.length - 1].id
													});
												}
											})(getbody)
										);
									...
									setImmediate(function(){getKeysCargo.push({limit:config.readKeySize});});
								}
							}
							callback();
						});
			...
			getDocsCargo.drain = function(){
				...
				var syncDB = function(){
					...
					switch (dbtype){
						case "mongo":
							if (!r.ready  || os.freemem() < (config.minMem*1048576)) {setImmediate(syncDB);return;}
							col.find({$or:[{status : {$exists: false}},{status : null}]}).limit(config.dbIncremental).toArray(
								function(err,data){
									if (err) {console.log("error while getting DB data");setImmediate(syncDB);return;}
									if (data.length<1){setImmediate(syncDB);return;}
