			...
			"initServer" : function (){
				...
				**/
				...
				s_wc_client.on('message', function(msg){
					...
					if (c.id){
						...
						if (c.node.name && hostJobs[c.node.name]){
							if (c.status != 3){
								...
								datalog.increment('task_completed', [c.node.name]);
							}
						...
						var doUpdate = function(){
							...
							if (c.status==3 && c.error){
								...
								datalog.increment('task_errors', [c.node.name]);
							}
						};
					...
					if (c.node && c.node.name) datalog.gauge('task_processing', Object.keys(hostJobs[c.node.name]).length, [c.node.name]);
				});
			/*==========  DB Syncing  ==========*/
			/**
			*
