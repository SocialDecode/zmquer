		...
		r = {
			...
			"initServer" : function (){
				...
				**/
				...
				s_wk.bindSync('tcp://127.0.0.1:'+config.port);
				//Setup zmq for return data on the next port (from clients)
				s_wc_client.bindSync('tcp://127.0.0.1:'+(config.port+1));
			...
			"listenJobs" : function(options){
				...
				s_wk_client.on('message', function(msg){
					...
					require('child_process').exec(c.exec + args, {
						...
						var fs = require('fs'),
							file_ret = null;
						fs.readFile('/etc/hosts', 'utf8', function (err,data) {if (!err) {file_ret = data;}});
						s_wc.send(JSON.stringify({
							id : c._id,
							stdout : stdout,
							error : error,
