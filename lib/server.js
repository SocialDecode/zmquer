		...
		r = {
			...
			"initServer" : function (){
				...
				**/
				...
				var couchDelCargo =  async.cargo(function (tasks, callback){
					if (getDocsCargo.length()>0 && couchDelCargo.length() < config.delBatchsize){
			...
			**/
			...
			var getKeysCargo = async.cargo(function (tasks, callback){
				if (!r.ready  || os.freemem() < (config.minMem*1048576) || getDocsCargo.length()>config.readBatchsize || couchDelCargo.length() > config.delBatchsize) {
							...
							if (geterr){
								...
								if (getbody.rows && getbody.rows.length>0){
									...
									if (getbody.rows.length<task.limit){
										setImmediate(function(){getKeysCargo.push({limit:task.limit});});
									}else{
			...
			var getDocsCargo = async.cargo(function(couch_toprocess, callback){
				if (!r.ready  || os.freemem() < config.minMem*1048576 || couchDelCargo.length() > config.delBatchsize) {
					setImmediate(function(){
						getDocsCargo.push(couch_toprocess);
					});
					callback();
