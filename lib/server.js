		...
		r = {
			...
			"initServer" : function (){
				...
				**/
				...
				var couchDelCargo =  async.cargo(function (tasks, callback){
					if (getDocsCargo.length()>0 && couchDelCargo.length() < couchDelCargo.payload){
			...
			**/
			...
			var getKeysCargo = async.cargo(function (tasks, callback){
				if (!r.ready  || os.freemem() < (config.minMem*1048576) || getDocsCargo.length()>getDocsCargo.payload || couchDelCargo.length() > couchDelCargo.payload) {
