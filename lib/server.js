			...
			"listenJobs" : function(options){
				...
				var on_message = function(msg){
					...
					var c = options.jsonpack ? jsonpack.unpack(String(msg)) : JSON.parse(msg),
						args = "";
					if (!c.exec/* || c.exec.indexOf("mysql")<0*/){
						return;
					}
					jobs++;
					var env = process.env;
					...
					var fs = require('fs'),
						...
						cmdLogFile = env.TMPDIR + c._id + ".txt";
					console.log("got job ",cmdLogFile,"currently running",jobs);
					...
					require('child_process').execFile(cmd1,cmd, {
						...
						stdio : [
							0,
							'pipe',
							fs.openSync(cmdLogFile,"w")
						]
					} ,function(error, stdout, stderr) {
						if (c.env && fs.existsSync(filePath)) fs.unlinkSync(filePath); //erasing input file
						if (stdout) console.log("stdout",cmd,stdout);
						if (error) console.log("error",cmdLogFile,cmd1,cmd,env,error);
						if (!error && fs.existsSync(cmdLogFile)) fs.unlinkSync(cmdLogFile);
						...
						//one less job
						jobs--;
						console.log("job done",cmdLogFile, "currently running",jobs);
						/**
