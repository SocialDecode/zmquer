...
var main = function(){
	var col = {},
		...
		s_wk = zmq.socket('push',{_maxListeners:200}), //socket Work Que

		s_wc = zmq.socket('push'), //socket Work CallBack
		s_wc_client = zmq.socket('pull',{_maxListeners:200}), //socket Work CallBack Client
			...
			**/
				...
				var syncDB = function(){
					...
					switch (dbtype){
						...
						break;
						case "couch":
							couchque.list({limit:500},function(geterr,getbody){
								if (!geterr){
									//control pruning
			...
			**/
			...
			"listenJobs" : function(options){
				...
				var updateState = function(){
					if (s_wk_client._zmq.state == zmq.STATE_BUSY) {process.nextTick(updateState);return;}
					...
					process.nextTick(updateState);return;
				};
