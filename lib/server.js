		...
		r = {
			...
			"initServer" : function (){
				...
				**/
				...
				var couchDelCargo =  async.cargo(function (tasks, callback){
					if (getDocsCargo.length()>0){
						setImmediate(function(){couchDelCargo.push(tasks);});
						callback();
					}else{
						var label = "droping "+tasks.length+" job(s)";
						console.time(label);
						var docs = [];
			...
			**/
			...
			var getKeysCargo = async.cargo(function (tasks, callback){
				if (!r.ready  || os.freemem() < (config.minMem*1048576) || getDocsCargo.length()>getDocsCargo.payload) {
			...
			**/
			...
			"listenJobs" : function(options){
				...
				var updateState = function(){
					if (s_wk_client._zmq.state !== zmq.STATE_CLOSED && ( (os.freemem()-(options.memJob*1048576*jobs)) < (options.minMem*1048576) )){
