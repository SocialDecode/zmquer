							...
							if (geterr){
								...
								if (getbody.rows && getbody.rows.length>0){
									...
									if (getbody.rows.length<task.limit){
										setImmediate(function(){getKeysCargo.push({limit:task.limit,descending:true});});
									}else{
										setImmediate(function(){
											getKeysCargo.push({limit:task.limit,startkey:getbody.rows[getbody.rows.length-1].id,descending:true});
										});
									}
								}else{
									setImmediate(function(){getKeysCargo.push({limit:task.limit,descending:true});});
