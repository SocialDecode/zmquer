	...
	var col = {},
		...
		r = {
			...
			"initServer" : function (){
				...
				**/
				...
				s_wc_client.on('message', function(msg){
					...
					if (c.id){
						var ids = [c.id,new ObjectID(c.id)];
						delete c.id;
						var doUpdate = function(){
							col.update({_id:{ $in : ids }},{$set: c},function(uperr,upcount){
								if (uperr) {
									console.log("Error while updating job",ids,uperr);
								}else{
									if (upcount<1) console.log("Unable to find job to update",ids);
								}
							});
						};
						if (config.dumpSuccess && c.status == 3 && !c.error){
							col.remove({_id:{ $in : ids }}, function(errdel,coldel){
								if (errdel) {
									console.log("Error while deleting job",ids,uperr);
									doUpdate();
								}else{
									if (coldel<1){
										console.log("Unable to find job to delete",ids);
										doUpdate();
									}
								}
							});
			...
			**/
				...
				var syncDB = function(){
					if (!r.ready || syncDBWorking  || os.freemem() < (config.minMem*1048576)) return;
					syncDBWorking = true;
					...
					col.find({
						...
						function(err,data){
							...
							for (var i=0;i<data.length;i++){
									...
									ids.push(data[i]._id);
									ids.push(new ObjectID(data[i]._id));
							}
							col.update({_id:{ $in : ids }},{$set: {status:1,updated : ((new Date()).getTime())/ 1000 | 0}},{multi:true}, function(){
								syncDBWorking = false;
							});
						});
