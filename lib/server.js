		...
		r = {
			...
			**/
							...
							couchque.list({},function(geterr,getbody){
								if (!geterr){
									...
									for (var x=0;x<getbody.rows.length;x++){
										if (!couch_processing[getbody.rows[x].id]) couch_toprocess.push(getbody.rows[x].id);
									}
									couch_toprocess.slice(0,config.dbIncremental);
									if (couch_toprocess.length<1){resyncDB();return;}
									process.stdout.write("q?"+couch_toprocess.length);
									couchque.list({
										keys:couch_toprocess,
										include_docs : true
									},function(bulkerr,bulkbody){
										for (var i=0;i<bulkbody.rows.length;i++){
											if (!bulkbody.rows[i].doc) continue;
											var id = bulkbody.rows[i].doc.id;
											couch_processing[id] = 1;
											bulkbody.rows[i].doc["idtype"] = "string";
											s_wk.send(jsonpack.pack(bulkbody.rows[i].doc));
				...
				var resyncDB = function (){
					...
					r.DBSyncInterval = setTimeout(syncDB,(dbtype=="couch") ? 5000:500);
					return;
				};
