	...
	var col = {},
		...
		httpCB = {
			syncData : function(cb){
				cb({processing:couch_processing,universe:jobsCounter});
			},
			resetJobs : function(){
				cleanedProc = {};
				...
				s_wc_client.on('message', function(msg){
					...
					if (c.id){
						...
						if (c.node.name && hostJobs[c.node.name]){
							if (c.status != 3){
								if (hostJobs[c.node.name]) {hostJobs[c.node.name][id] = true;}else{hostJobs[c.node.name] = {id:true};}
							}else{
			...
			**/
				...
				var syncDB = function(){
					...
					switch (dbtype){
						...
						break;
						case "couch":
							couchque.list({limit:10000},function(geterr,getbody){
								if (!geterr){
									//control pruning
