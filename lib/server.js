				...
				**/
				...
				if (config.datadog){
					...
					datalog.legauge = function(type,host){
						...
						if (!datalog.legauges[key] || (now - datalog.legauges[key].age > 300)){
							...
							datalog.legauges[key].count++;
						}
						datalog.gauge(type, datalog.legauges[key].count, ["worker:"+host]);
					};
				}
			...
			"syncJobs" : function(host,jobs){
				if (host && Array.isArray(jobs)){
					...
					s_wk._outgoing = s_wk._outgoing.filter(function(item){
						jobs.forEach(function(el){
							return  (item.toString('utf8').indexOf(el) == -1);
						});
					});
				}
			},
